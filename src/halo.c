...
void circle_layer_update_callback(Layer *layer, GContext* ctx) {
	...
	const GPoint dividerStartOne = ((GPoint) {(bounds.size.w/2)-22, (bounds.size.h/2)+19});
	const GPoint dividerEndOne = ((GPoint) {(bounds.size.w/2)+22, (bounds.size.h/2)+19});
	const GPoint dividerStartTwo = ((GPoint) {(bounds.size.w/2)-20, (bounds.size.h/2)+21});
	const GPoint dividerEndTwo = ((GPoint) {(bounds.size.w/2)+20, (bounds.size.h/2)+21});
...
void handle_second_tick(struct tm *tick_time, TimeUnits units_changed) {
	...
	time_format = "%M";
	strftime(minute_text, sizeof(time_text), time_format, tick_time);
...
void handle_init(void) {
	...
	text_hour_layer = text_layer_create((GRect) { .origin = { 0, 40 }, .size = { window_bounds.size.w, window_bounds.size.h/2 } });

	// Create a text layer in the center of the screen for minutes (shift origin to be below hourly text)
	text_minute_layer = text_layer_create((GRect) { .origin = { 0, 105 }, .size = { window_bounds.size.w, window_bounds.size.h/2 } });
